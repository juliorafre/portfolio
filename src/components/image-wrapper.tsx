    ...
    <>
      <div className={wrapperClassName}>
        ...
        >
          <MotionImage
            /*layoutId={imageId}*/
            className={cn('h-full w-full object-cover rounded-lg', imageProps.className)}
            src={imageProps.src}
            alt={imageProps.alt}
            width={imageProps.width}
            height={imageProps.height}
          ...
          <Dialog.Root>
            <Dialog.Portal forceMount>
              ...
              <Dialog.Overlay key="overlay-wrapper">
                <motion.div
                  key="overlay"
                  className={cn('fixed inset-0 bg-black/65 backdrop-blur-xs', overlayClassName)}
                  initial={{opacity: 0}}
                  animate={{opacity: 1}}
                  exit={{opacity: 0}}
              ...
              {/* Content */}
              <Dialog.Content
                className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[min(90vh,90vw)] sm:h-[min(80vh,80vw)] flex items-center justify-center">
                ...
                <motion.div
                  layoutId={containerId} key="image-wrapper"
                  className={cn('relative w-full h-full rounded-lg overflow-hidden', modalClassName)}
                  transition={{
                    type: 'spring',
                    bounce: 0.3,
                    duration: 0.45,
                ...
                >
                  <Dialog.Close asChild>
                    <motion.button
                      className={cn('absolute top-4 right-4 cursor-pointer border border-white/15 rounded-full bg-white/50 p-2 shadow-md backdrop-blur-2xl active:scale-95 hover:scale-101 transition-all duration-200')}
                  ...
                  <MotionImage
                    /*layoutId={imageId}*/
                    className={cn('object-contain size-full', imageProps.className)}
                    src={imageProps.src}
                    alt={imageProps.alt}
                    width={imageProps.width}
                    height={imageProps.height}
